...
const { startOnboarding, handleOnboardingStep, askSpeciesJIT, askVillageJIT } = require('../services/onboarding');
...
async function handleTextMessage(phone, text) {
  ...
  if (!farmer || !farmer.onboarding_complete) {
    // If already mid-onboarding flow, continue it
    if (isInFlow(phone)) {
      const state = getState(phone);
      if (state.flow === 'onboarding') {
        const pending = await handleOnboardingStep(phone, text);
        // Onboarding just completed — auto-process any pending message or image
        if (pending?.message) {
          await handleTextMessage(phone, pending.message);
        } else if (pending?.imageBuffer) {
          await processPendingImage(phone, pending.imageBuffer, pending.imageMimeType);
        }
        return;
      }
    }

    // Brand new farmer — acknowledge their message, store it, then start onboarding.
    // The question will be automatically answered once they complete 2 taps.
    await sendTextMessage(
      phone,
      `👋 Hi! I\'d love to help with that.\n\nFirst, let me get you set up in *2 quick taps* — then I\'ll answer your question automatically. 🚀`
    );
    await startOnboarding(phone, { pendingMessage: text });
    return;
  }
  ...
  if (isInFlow(phone)) {
    ...
    if (flow === 'awaiting_jit_pond_size') {
      let pondSize = null;
      if (text.includes('less') || text.includes('<1') || text.includes('size_small') || text.includes('jit_size_s')) pondSize = 'less_than_1_acre';
      else if ((text.includes('1') && text.includes('3')) || text.includes('size_medium') || text.includes('jit_size_m')) pondSize = '1_3_acres';
      else if (text.includes('more') || text.includes('>3') || text.includes('size_large') || text.includes('jit_size_l')) pondSize = 'more_than_3_acres';
    ...
    // JIT COLLECTION: Species / Farm Type
    // Triggered when a feature needs pond.species or farmer.farm_type but it's null
    if (flow === 'awaiting_jit_species') {
      const { pendingTopic } = getState(phone); // remember what the user originally wanted
      let farmType = null;
      let species = null;

      if (normalizedText.includes('shrimp') || normalizedText === 'jit_shrimp') { farmType = 'shrimp'; species = 'vannamei'; }
      else if (normalizedText.includes('fish') || normalizedText === 'jit_fish') { farmType = 'fish'; species = 'tilapia'; }
      else if (normalizedText.includes('both') || normalizedText === 'jit_both') { farmType = 'both'; species = 'vannamei_and_tilapia'; }

      if (!farmType) {
        // Unrecognised — re-prompt
        await askSpeciesJIT(phone, lang);
        return;
      }

      // Persist species to pond and farm_type to farmer
      const { updatePond, updateFarmer } = require('../models/database');
      const pond = await getFirstPondByFarmer(farmer.id);
      await Promise.all([
        updatePond(pond.id, { species }),
        updateFarmer(farmer.id, { farm_type: farmType }),
      ]);

      clearState(phone);

      // Re-route to the original topic the farmer wanted
      if (pendingTopic) {
        await startEventFollowUp(phone, farmer.id, pendingTopic);
      }
      return;
    }

    // JIT COLLECTION: Village
    if (flow === 'awaiting_jit_village') {
      const { pendingTopic } = getState(phone);
      if (text.length < 3) {
        await askVillageJIT(phone, lang);
        return;
      }
      const { updateFarmer } = require('../models/database');
      await updateFarmer(farmer.id, { village: text.trim() });
      clearState(phone);
    ...
    if (flow === 'onboarding') {
      const pending = await handleOnboardingStep(phone, text);
      // Onboarding just completed with pending question/image — auto-process
      if (pending?.message) {
        await handleTextMessage(phone, pending.message);
      } else if (pending?.imageBuffer) {
        await processPendingImage(phone, pending.imageBuffer, pending.imageMimeType);
...
/**
 * Auto-process a pending image buffer after onboarding completes.
 * Called when a farmer sent a photo before they were onboarded.
 */
async function processPendingImage(phone, imageBuffer, imageMimeType) {
  const farmer = await getFarmerByPhone(phone);
  if (!farmer) return;
  ...
  await sendTextMessage(phone,
    lang === 'Telugu' ? '🔬 మీ ఫోటో విశ్లేషిస్తున్నాను...' :
    lang === 'Hindi'  ? '🔬 आपकी फ़ोटो का विश्लेषण कर रहा हूँ...' :
                        '🔬 Analysing your photo now...'
  );
  ...
  try {
    ...
    try {
      const pond = await getFirstPondByFarmer(farmer.id);
      if (pond) pondContext = { species: pond.species, pondSize: pond.pond_size };
    } catch (_) {}

    const analysis = await analyzeImage(imageBuffer, lang, pondContext);
    await sendTextMessage(phone, analysis.text);

    // Save to chat history
    await saveChatHistory({
      farmer_id: farmer.id,
      message: '[Photo sent before registration]',
    ...
    logger.error('Pending image analysis failed', { phone, error: err.message });
    await sendTextMessage(phone,
      lang === 'Telugu' ? '❌ ఫోటో విశ్లేషణ విఫలమైంది. దయచేసి మళ్ళీ ఫోటో పంపండి.' :
      lang === 'Hindi'  ? '❌ फ़ोटो विश्लेषण विफल हुआ। कृपया फ़ोटो दोबारा भेजें।' :
                          '❌ Photo analysis failed. Please resend your photo.'
    );
  }
}
...
async function handleImageMessage(phone, imageData) {
  ...
  if (!farmer || !farmer.onboarding_complete) {
    // Download the image immediately before the media token expires,
    // store the buffer in onboarding state, and process it after setup.
    await sendTextMessage(
      phone,
      `📸 Got your photo! I can analyse it for disease detection.\n\nFirst, let me get you set up in *2 quick taps* — then I\'ll analyse it automatically. 🚀`
    );
    try {
      const imageBuffer = await downloadMedia(imageData.id);
      await startOnboarding(phone, {
        pendingImageBuffer: imageBuffer,
        pendingImageMimeType: imageData.mime_type || 'image/jpeg',
      });
    } catch (downloadErr) {
      logger.warn('Could not pre-download image for pending onboarding', { error: downloadErr.message });
      // Start onboarding without pending image — farmer will need to resend
      await startOnboarding(phone);
    }
    return;
  }
